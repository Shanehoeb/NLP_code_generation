...
def get_dataset(dataset_name, metadata=False, synthetic_train_path=None):
    ...
    elif dataset_name=='codeparrot-clean':
        dataset = load_dataset('codeparrot/codeparrot-clean-train')
        train_ds = dataset['train']
        train_val_ds = train_ds.train_test_split(test_size=1000, seed=42)
        train_val_ds['valid'] = train_val_ds['test']

        dataset = process_codeparrot_dataset(train_val_ds)
...
def process_codeparrot_dataset(dataset):
    def process_codeparrot_text(example):
        return {'text': PreTrainedTokenizerBase.clean_up_tokenization(example["content"].strip())}
    dataset = dataset.map(process_codeparrot_text,  remove_columns=['repo_name', 'path', 'copies', 'size', 'license', 'hash', 'line_mean', 'line_max', 'alpha_frac', 'autogenerated'])
    return dataset
